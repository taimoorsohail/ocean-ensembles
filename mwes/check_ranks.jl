...
using OceanEnsembles.OutputWrangling: read_bathymetry
...
@time bottom_height = regrid_bathymetry(underlying_grid, ETOPOmetadata;
				                  ...
				                  major_basins = 1)
# view(bottom_height, 73:78, 88:89, 1) .= -1000 # open Denmark strait
...
files = glob("global_*fields*$(resolution)*_RYF_iteration*.jld2", output_path * "saved_fields/")
...
for file in files
    ...
    push!(prefixes, joinpath(output_path * "saved_fields/", prefix))
end
...
bottom_height2 = read_bathymetry(unique_prefixes[1]*"_iteration0", [0,1,2,3])

fig = Figure(size = (800, 600))

ax1 = Axis(fig[1, 1], title = "Bathymetry", xlabel = "Longitude", ylabel = "Latitude")

hm = heatmap!(ax1, interior(grid.immersed_boundary.bottom_height)[:,:,1] - bottom_height2; colormap = Reverse(:bwr), colorrange = (-1,1))
Colorbar(fig[1,2], hm; label = "Depth (m)")

save(figdir * "Bathymetry_diff.png", fig)