...
source_field = Field{Face, Center, Center}(underlying_source_grid)
destination_field = Field{Face, Center, Center}(underlying_destination_grid)
...
@time W_cons = permutedims(regridder_weights!(source_field, destination_field; method = "conservative"))

dst_cons = LinearAlgebra.mul!(vec(interior(destination_field)[1,:,:]), W_cons, vec(interior(source_field*Ax)[1,:,:]))
dst_cons_2 = LinearAlgebra.mul!(vec(interior(dst_cons_2)[:,:,1]), W_cons, vec(interior(dst_cons)[:,:,1]))
