...
Smetadata = Metadata(:salinity;    dates, dataset = dataset, dir=data_path)
...
set!(ocean.model, T=Metadatum(:temperature; dataset, date = first(dates), dir = data_path),
                  S=Metadatum(:salinity;    dataset, date = first(dates), dir = data_path))
...
atmosphere = JRA55PrescribedAtmosphere(arch; backend=backend, include_rivers_and_icebergs=true)
